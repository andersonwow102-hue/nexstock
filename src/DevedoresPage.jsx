...
function PagamentoForm({ detalhe, onCancelar, onConcluir }) {
  ...
  return <Modal titulo="Registrar pagamento" subtitulo="Pagamentos parciais e integrais são validados pelo banco." onFechar={onCancelar} bloqueado={enviando} footer={<><button className="btn-secundario" disabled={enviando} onClick={onCancelar}>Cancelar</button><button className="btn-primario" disabled={enviando} onClick={salvar}>{enviando?"Processando...":confirmando?"Confirmar pagamento":"Revisar pagamento"}</button></>}>
...
export default function DevedoresPage({ perfilAtual }) {
  ...
  return <div className="dev-page">
    ...
    <section className="dev-kpis" aria-label="Resumo de devedores">
      <div className="dev-kpi-aberto"><span>Dívidas abertas</span><b>{kpis.abertas}</b></div><div><span>Total original</span><b>{moeda(kpis.original)}</b></div><div><span>Total negociado</span><b>{moeda(kpis.negociado)}</b></div><div className="dev-kpi-pago"><span>Total pago</span><b>{moeda(kpis.pago)}</b></div><div className="dev-kpi-saldo"><span>Saldo atual</span><b>{moeda(kpis.saldo)}</b></div><div className="dev-kpi-vencido"><span>Vencidas</span><b>{kpis.vencidas}</b></div><div className="dev-kpi-quitado"><span>Quitadas</span><b>{kpis.quitadas}</b></div>
    </section>
    <section className="dev-filtros" aria-label="Filtros de devedores">
      <div className="dev-filtros-cabecalho"><div><h3>Pesquisar e filtrar</h3><p>Refine a relação de devedores pelos critérios abaixo.</p></div><button className="btn-secundario" onClick={()=>{setBusca("");setFiltros({situacao:"",modalidade:"",gerente:"",forma:"",marcador:"",periodo:""});}}>Limpar filtros</button></div>
      <div className="dev-filtros-grid">
        <Campo label="Busca" className="dev-filtro-busca"><input placeholder="Buscar por nome, fantasia, telefone, cidade ou gerente" value={busca} onChange={e=>setBusca(e.target.value)}/></Campo>
        <Campo label="Situação"><select value={filtros.situacao} onChange={e=>setFiltros({...filtros,situacao:e.target.value})}><option value="">Todas as situações</option>{["aberta","negociada","parcialmente_paga","vencida","quitada"].map(s=><option key={s} value={s}>{situacaoApresentacao(s)}</option>)}</select></Campo>
        <Campo label="Modalidade"><select value={filtros.modalidade} onChange={e=>setFiltros({...filtros,modalidade:e.target.value})}><option value="">Todas as modalidades</option>{dados.modalidades.map(m=><option key={m.id} value={m.id}>{m.nome}</option>)}</select></Campo>
        <Campo label="Gerente responsável"><select value={filtros.gerente} onChange={e=>setFiltros({...filtros,gerente:e.target.value})}><option value="">Todos os gerentes</option>{gerentes.map(g=><option key={g}>{g}</option>)}</select></Campo>
        <Campo label="Forma de pagamento"><select value={filtros.forma} onChange={e=>setFiltros({...filtros,forma:e.target.value})}><option value="">À vista ou parcelada</option><option value="vista">À vista</option><option value="parcelada">Parcelada</option></select></Campo>
        <Campo label="Marcador"><select value={filtros.marcador} onChange={e=>setFiltros({...filtros,marcador:e.target.value})}><option value="">Todos os registros</option><option value="vencidas">Somente vencidas</option><option value="quitadas">Somente quitadas</option></select></Campo>
        <Campo label="Mês de registro"><input type="month" value={filtros.periodo} onChange={e=>setFiltros({...filtros,periodo:e.target.value})}/></Campo>
